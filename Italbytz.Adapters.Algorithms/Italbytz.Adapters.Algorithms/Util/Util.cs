...
namespace Italbytz.Adapters.Algorithms.Util
{
    ...
    public static class Util
    {
         ...
         */
        public static T First<T>(List<T> l) => l[0];
        ...
        private static int Min(int i, int j) => i > j ? j : i;

        private static int Max(int i, int j) => i < j ? j : i;

        public static int Max(int i, int j, int k) => Max(Max(i, j), k);

        public static int Min(int i, int j, int k) => Min(Min(i, j), k);
        ...
        private static double Log2(double d) => Math.Log(d) / Math.Log(2);
        ...
        public static double GenerateRandomDoubleBetween(double lowerLimit,
            double upperLimit) =>
            lowerLimit + (upperLimit - lowerLimit) * R.NextDouble();
    }
}